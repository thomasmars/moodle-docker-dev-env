...
function initDialogHandler(name, val) {
    ...
    builder = builder instanceof _clientFunctionBuilder2.default ? builder : null;

    if (builder) {
        if (builder instanceof _selectorBuilder2.default) throw new _testRun.SetNativeDialogHandlerCodeWrongTypeError(builder.callsiteNames.instantiation);

        fn = fn.with(options);

        builder = fn[_builderSymbol2.default];
    } else {
        var functionType = typeof fn === 'undefined' ? 'undefined' : (0, _typeof3.default)(fn);

        if (functionType !== 'function') throw new _testRun.SetNativeDialogHandlerCodeWrongTypeError(functionType);

        builder = new _clientFunctionBuilder2.default(fn, options, {
