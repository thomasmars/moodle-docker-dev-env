...
function purify_html($text, $options = array()) {
    ...
    if (!file_exists($cachedir)) {
        ...
        make_localcache_directory('HtmlReportPurifier', false);
    ...
    if (!array_key_exists($type, $caches)) {
        $caches[$type] = cache::make('core', 'HtmlReportPurifier', array('type' => $type));
    }
    $cache = $caches[$type];
    ...
    if (empty($purifiers[$type])) {
        ...
        $purifier = new HtmlReportPurifier($config);
